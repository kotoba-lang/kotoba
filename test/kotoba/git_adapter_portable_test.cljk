(ns kotoba.git-adapter-portable-test
  "The genuinely portable slice of kotoba.git-adapter-test: everything that
  goes through the pure `plan`/`parse-status`/`execute!` surface via an
  injected IProcess port, with no process/filesystem I/O of its own.
  `launcher-executes-git-end-to-end` stays in kotoba.git-adapter-test (.clj)
  because it shells out to a real `git` subprocess through
  kotoba.launcher/dispatch and touches java.nio.file -- that is a real JVM
