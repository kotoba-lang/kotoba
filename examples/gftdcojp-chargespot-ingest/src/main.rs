...
use kotoba_auth::{
    ...
    Cacao, CacaoHeader, CacaoPayload, CacaoSig,
};
use kotoba_core::cid::KotobaCid;
use kotoba_kqe::{delta::Delta, quad::{Quad, QuadObject}};
use rand::rngs::OsRng;
...
const COMPETITORS: &[(&str, &str, &str, &str, &str)] = &[
    ("energy-monster",  "cn",  "怪兽充電 (Energy Monster)", "dominant",   "NASDAQ上場(EM)、国内最大手"),
    ("meituan-charge",  "cn",  "美団充電 (Meituan)",        "dominant",   "怪兽買収後に国内最大規模"),
    ("zhuman-tech",     "cn",  "竹芒科技",                   "active",     "街電＋搜電合併、数億台規模"),
...
const STRUGGLE_FACTORS: &[(&str, &str, &str, i64)] = &[
    ("f1", "user_behavior",     "車移動中心・スマホ依存度低・自分でバッテリーを持つ文化 (欧米)", 5),
    ("f2", "unit_economics",    "観光地特化=季節変動大・キオスク設置コスト固定・回転率低で採算未達", 5),
    ("f3", "venue_negotiation", "商業施設との設置交渉コスト高・リベニューシェア構造が複雑 (欧米)", 4),
    ("f4", "payment_friction",  "クレカ登録・アプリDLの離脱率高・デポジット取得コスト (欧米・東南アジア)", 4),
    ("f5", "theft_return",      "返却率低・デポジット回収の法的コスト高 (欧米・東南アジア一部)", 3),
];
...
fn graph_cid_val() -> KotobaCid { cid(GRAPH_ID) }

fn quad(subject: &str, predicate: &str, object: QuadObject) -> Quad {
    Quad { graph: graph_cid_val(), subject: cid(subject), predicate: predicate.to_string(), object }
}
...
impl IngestIdentity {
    ...
    fn make_cacao(&self, server_url: &str) -> Result<String> {
        ...
        let cacao = Cacao {
            ...
            p: payload,
            s: CacaoSig { t: String::new(), s: String::new() }, // filled below
        };
        ...
        let signed = Cacao {
            s: CacaoSig { t: "EdDSA".to_string(), s: sig_b64 },
            ..cacao
        };

        // Serialize to DAG-CBOR → standard base64
        let mut cbor_bytes = Vec::new();
        ciborium::into_writer(&signed, &mut cbor_bytes)
            .context("CACAO DAG-CBOR serialize")?;
...
async fn xrpc_quad_create(
    ...
    let resp = client.post(&url).json(&body).send().await.context("quad.create POST")?;
    let status = resp.status();
    if !status.is_success() {
...
async fn xrpc_commit(
    ...
    let resp = client.post(&url).json(&body).send().await.context("commit.store POST")?;
    let status = resp.status();
...
async fn main() -> Result<()> {
    ...
    let server = std::env::var("KOTOBA_SERVER")
    ...
    let assert_quads: Vec<&Quad> = facts.iter()
    ...
    for q in &assert_quads {
        ...
        xrpc_quad_create(
            &client, &server,
            &graph_mb, &subj_mb, &q.predicate, &obj_str,
            &cacao_b64,
        ).await?;
        ok += 1;
        if ok % 20 == 0 {
    ...
    println!("  → kotobase pin queued (fire-and-forget to kotobase.gftd.ai if KOTOBA_PIN_TOKEN set)");

    // 5. Verify via MCP kotoba_graph_query
    println!("\nverifying via MCP kotoba_graph_query …");
    let query_result = mcp_call(
        &client, &mcp_url, &jwt, 1,
        "kotoba_graph_query",
        json!({ "graph": graph_mb, "predicate": "bma/namespace", "limit": 5 }),
    ).await?;
    println!("MCP query result: {}", serde_json::to_string_pretty(&query_result)?);
