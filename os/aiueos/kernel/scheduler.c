...
struct aiueos_task { uint64_t *saved_stack; uint64_t switches; };
...
static void task_loop(volatile uint64_t *runs, uint8_t marker) {
  for (;;) { ++*runs; if (*runs == 1) debug_byte(marker); __asm__ volatile("hlt"); }
}
__attribute__((noreturn)) static void task_a(void) {
  task_loop(&aiueos_scheduler_task_a_runs, 'A'); __builtin_unreachable();
}
__attribute__((noreturn)) static void task_b(void) {
  task_loop(&aiueos_scheduler_task_b_runs, 'B'); __builtin_unreachable();
}
