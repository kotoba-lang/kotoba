...
pub use p2p::{
    input_topic, state_topic, ChannelGossipBus, GossipBus, P2pAuthority, P2pClient,
};
pub use protocol::{
...
mod tests {
    ...
    fn ground_truth(players: &[PlayerId], log: &[(PlayerId, u64, u32)], ticks: u64) -> [u8; 32] {
        ...
        for (p, t, b) in log {
            eng.add_input(*p, Tick(*t), Input { buttons: *b, axes: vec![] });
        }
    ...
    fn protocol_cbor_roundtrips() {
        ...
        let frame = ClientMsg::Input(InputFrame {
            ...
            tick: Tick(9),
            seq: 100,
            input: Input { buttons: 0b1010, axes: vec![0.5, -0.25] },
        });
    ...
    fn nan_inf_axes_cannot_desync() {
        ...
        let mk = |axes: Vec<f32>| {
            let mut e =
                RollbackEngine::new(CounterSim::new(), players.clone(), 3, &player_cfg(1));
...
"#;
    ...
    fn wasm_guest_rollback_reconverges() {
        ...
        let mut truth = mk();
        truth.add_input(PlayerId(0), Tick(0), Input { buttons: 3, axes: vec![] });
        truth.add_input(PlayerId(1), Tick(1), Input { buttons: 5, axes: vec![] });
        truth.advance_to(Tick(10));
        let truth_hash = truth.state_hash();

        let mut eng = mk();
        eng.add_input(PlayerId(0), Tick(0), Input { buttons: 3, axes: vec![] });
        eng.advance_to(Tick(4)); // player 1's tick-1 input missing → mispredict
        let rolled = eng.add_input(PlayerId(1), Tick(1), Input { buttons: 5, axes: vec![] });
    ...
    fn wasm_component_rollback_reconverges() {
        ...
        assert_eq!(eng.state_hash(), truth_hash, "component rollback reconverges");
        assert!(eng.stats().rollbacks >= 1);
    }
    ...
    async fn boxed_component_sim_drives_a_room() {
        ...
        room.submit_input(PlayerId(0), Tick(0), 1, Input { buttons: 4, axes: vec![] });
        let before = room.state_hash();
        room.tick_once(|_| String::new());
        let after = room.state_hash();
        assert_ne!(before, after, "boxed component sim must advance state via the room");
    ...
    fn wasm_component_matches_cross_impl_snapshot_vector() {
        ...
        sim.step(
            Tick(0),
            &[
                (PlayerId(0), Input { buttons: 3, axes: vec![] }),
                (PlayerId(1), Input { buttons: 5, axes: vec![] }),
            ],
        );
        sim.step(
            Tick(1),
            &[
                (PlayerId(0), Input { buttons: 1, axes: vec![] }),
                (PlayerId(1), Input { buttons: 2, axes: vec![] }),
            ],
        );
        let hex: String = sim.snapshot_durable().iter().map(|b| format!("{b:02x}")).collect();
        assert_eq!(
    ...
    fn wasm_guest_durable_snapshot_restores() {
        ...
        a.init(7, &[]);
        a.step(Tick(0), &[(PlayerId(0), Input { buttons: 4, axes: vec![] })]);
    ...
    fn deterministic_same_inputs_same_hash() {
        ...
        let log = [(PlayerId(0), 0, 3), (PlayerId(1), 1, 5), (PlayerId(0), 4, 2)];
    ...
    fn no_rollback_when_inputs_arrive_in_order() {
        ...
        for t in 0..5u64 {
            ...
            eng.add_input(PlayerId(0), Tick(t), Input { buttons: t as u32, axes: vec![] });
            eng.add_input(PlayerId(1), Tick(t), Input { buttons: 1, axes: vec![] });
            eng.advance(1);
        }
        assert_eq!(eng.stats().rollbacks, 0, "in-order inputs must not roll back");
    ...
    fn rollback_reconverges_to_ground_truth() {
        ...
        let log = [(PlayerId(0), 0u64, 3u32), (PlayerId(1), 1, 5), (PlayerId(0), 4, 2)];
        ...
        eng.add_input(PlayerId(0), Tick(4), Input { buttons: 2, axes: vec![] });
        eng.advance_to(Tick(10));
    ...
    fn correct_prediction_no_rollback() {
        ...
        eng.add_input(PlayerId(0), Tick(0), Input { buttons: 4, axes: vec![] });
        eng.advance_to(Tick(3)); // ticks 1,2 predict "buttons:4" (repeat last)
        // confirm tick 1 with the SAME predicted value
        let rolled = eng.add_input(PlayerId(0), Tick(1), Input { buttons: 4, axes: vec![] });
    ...
    fn dynamic_roster_join_is_deterministic_and_order_independent() {
        ...
        a.add_input(PlayerId(0), Tick(0), Input { buttons: 1, axes: vec![] });
        a.add_roster_event(PlayerId(1), Tick(3), true);
        a.add_input(PlayerId(1), Tick(3), Input { buttons: 10, axes: vec![] });
        a.advance_to(Tick(5));
        ...
        b.add_input(PlayerId(1), Tick(3), Input { buttons: 10, axes: vec![] });
        b.add_input(PlayerId(0), Tick(0), Input { buttons: 1, axes: vec![] });
        b.advance_to(Tick(5));
    ...
    fn dynamic_capacity_grows_deterministically_for_high_player_ids() {
        ...
        let run = |swap: bool| {
            ...
            if swap {
                e.add_input(PlayerId(5), Tick(0), Input { buttons: 9, axes: vec![] });
                e.add_roster_event(PlayerId(5), Tick(0), true);
                e.add_input(PlayerId(0), Tick(0), Input { buttons: 2, axes: vec![] });
            } else {
                e.add_roster_event(PlayerId(5), Tick(0), true);
                e.add_input(PlayerId(0), Tick(0), Input { buttons: 2, axes: vec![] });
                e.add_input(PlayerId(5), Tick(0), Input { buttons: 9, axes: vec![] });
        ...
        base.add_input(PlayerId(0), Tick(0), Input { buttons: 2, axes: vec![] });
        base.advance_to(Tick(2));
        assert_ne!(run(false), base.state_hash(), "high-id player must change state");
    }

    #[test]
    fn late_roster_event_rolls_back_and_reconverges() {
        // Ground truth: player 1 active (and inputs) from tick 1.
        let mut truth = RollbackEngine::new(CounterSim::new(), vec![PlayerId(0)], 0, &player_cfg(8));
        truth.add_roster_event(PlayerId(1), Tick(1), true);
        truth.add_input(PlayerId(0), Tick(0), Input { buttons: 2, axes: vec![] });
        truth.add_input(PlayerId(1), Tick(1), Input { buttons: 7, axes: vec![] });
        ...
        eng.add_input(PlayerId(0), Tick(0), Input { buttons: 2, axes: vec![] });
        eng.advance_to(Tick(4));
        assert!(eng.add_roster_event(PlayerId(1), Tick(1), true), "past roster change rolls back");
        eng.add_input(PlayerId(1), Tick(1), Input { buttons: 7, axes: vec![] });
        eng.advance_to(Tick(6));
        assert_eq!(eng.state_hash(), th, "roster + input rollback reconverge to truth");
    ...
    async fn t2_signaling_is_relayed_with_from_and_to() {
        ...
        room.relay_signal(PlayerId(1), PlayerId(2), SignalPayload::Offer("v=0...".into()));

        let got = drain(&mut rx);
        let signal = got.iter().find_map(|m| match m {
            ServerMsg::Signal { from, to, payload, .. } => Some((*from, *to, payload.clone())),
            _ => None,
        });
        assert_eq!(
            signal,
            Some((PlayerId(1), PlayerId(2), SignalPayload::Offer("v=0...".into()))),
    ...
    async fn room_dynamic_join_grows_roster_at_effective_tick() {
        ...
        assert!(room.roster().is_empty(), "not yet active at the current tick");

        room.tick_once(|_| String::new()); // advance to tick 1
        assert_eq!(room.roster(), vec![PlayerId(2)], "active at the effective tick");
    ...
    async fn room_bus_broadcasts_final_confirms_and_snapshots() {
        ...
        room.submit_input(PlayerId(0), Tick(0), 1, Input { buttons: 1, axes: vec![] });
        ...
        assert!(snap.is_some(), "durable snapshot must fire at snapshot_interval");
        ...
        assert_eq!(forwarded_inputs, 1, "the submitted input is forwarded immediately");
        assert_eq!(confirms, 2, "only finalized (past-horizon) ticks are confirmed");
    ...
    async fn confirm_is_never_emitted_for_a_tick_that_later_rolls_back() {
        ...
        room.submit_input(PlayerId(0), Tick(0), 1, Input { buttons: 3, axes: vec![] });
        ...
        for msg in drain(&mut rx) {
            if let ServerMsg::Confirm(c) = msg {
                assert!(c.tick.0 != 1, "tick 1 confirmed before its inputs were final");
            }
        }

        // Late correction for tick 1 → rollback.
        let rolled = room.submit_input(PlayerId(1), Tick(1), 1, Input { buttons: 5, axes: vec![] });
