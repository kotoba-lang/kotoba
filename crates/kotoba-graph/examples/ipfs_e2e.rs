...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Node B store: {} blocks (ProllyTree EAVT/AEVT/AVET/VAET + commit)", peer_cids.len());
    ...
    let dist_store = Arc::new(
        DistributedBlockStore::new(Arc::clone(&local_a), vec![])
    ...
    println!("Node A import_commit({}) → {}", commit_cid.to_multibase(), imported);

    print_header("Phase 4: SPARQL BGP — ?s <role> \"admin\" (Node A, cold path)");

    let admins = node_a.cold_query_sparql_bgp(
        &graph,
    ...
    let is_admin = node_a.sparql_ask(
        &graph,
    ...
    let described = node_a.sparql_describe(
        &graph,
        &format!("DESCRIBE <cid:{alice_mb}>"),
    ).await?;
    println!("Result: {} quads", described.len());
    for q in &described {
        println!("  {} {} {:?}", q.subject.to_multibase(), q.predicate, q.object);
    }

    print_header("Phase 7: SPARQL CONSTRUCT — relabel admins");

    let constructed = node_a.sparql_construct(
        &graph,
    ...
    println!("Resources: quad:read + graph/{graph_mb}");

    let authed = node_a.sparql_describe_authed(
        &graph,
        &format!("DESCRIBE <cid:{alice_mb}>"),
        &chain,
    ).await;
    ...
    let denied = node_a.sparql_describe_authed(
        &wrong,
        &format!("DESCRIBE <cid:{alice_mb}>"),
        &chain,
    ).await;
    println!("Cross-graph DESCRIBE (wrong target): {}",
        if denied.is_err() { "denied ✓" } else { "ALLOWED (bug)" });

    print_header("Phase 9: SPARQL 1.1 SERVICE — federate to Node B's graph from Node A");

    let federated = node_a.cold_query_sparql_bgp(
        &graph,
        &format!(
            "SELECT * WHERE {{ SERVICE <cid:{graph_mb}> {{ ?s <role> \"admin\" }} }}"
        ),
    ).await?;
    println!("Federated result (SERVICE <cid:{graph_mb}>): {} quads", federated.len());

    print_header("Summary");

    println!("Node B: {} blocks committed (content-addressed via blake3 + dag-cbor)", peer_cids.len());
    println!("Node A: imported commit, queried graph via DistributedBlockStore (5 SPARQL forms tested)");
