...
//!   KOTOBA_OPERATOR_DID         operator DID for the Engi wallet (default did:web:etzhayyim.com)
        ...
        (Some(evm), kaizen_feed) => {
            ...
            match driver.tick(&evm, &feed, &from_block, &to_block, epoch, pool) {
                Ok((minted, result)) => {
                    ...
                    let engi = Engi::from_env(operator);
                    let credited = SocialEconomyDriver::settle_to_wallet(&engi, &result).await;
...
async fn run_fake_demo(graph: KotobaCid, epoch: u64, pool: i64) {
    ...
    let engi = Engi::from_env("did:key:demo-operator".to_string());
    let credited = SocialEconomyDriver::settle_to_wallet(&engi, &result).await;
    println!(
        "FAKE: credited {credited} mKOTO to peggy; balance now {}.",
        engi.balance(&peggy.to_string()).await
    );
