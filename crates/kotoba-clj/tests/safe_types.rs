...
//! arithmetic. safe Kotoba rejects a numeric operator applied to a non-numeric
...
#[test]
fn bytes_alloc_negative_literal_is_rejected() {
    // A negative capacity would become a huge unsigned cap (defeats the overflow
    // guard); reject the literal form statically.
    denied_type(compile_safe_kotoba(
        r#"(defn run [] (bytes-alloc -1))"#,
        &Policy::deny_all(),
    ));
}

#[test]
fn bytes_alloc_nonnegative_literal_compiles() {
    let wasm = compile_safe_kotoba(
        r#"(defn run [] (bytes-len (bytes-alloc 4)))"#,
        &Policy::deny_all(),
    )
    .expect("a non-negative bytes-alloc must compile");
    assert!(wasm.starts_with(b"\0asm"));
}

#[test]
fn bitwise_ops_on_string_literal_are_rejected() {
    // Bit ops are integer-only; a string handle in one is silent handle math.
    for op in [
        "bit-and",
        "bit-or",
        "bit-xor",
        ...
        let src = format!(r#"(defn run [] ({op} "x" 1))"#);
        denied_type(compile_safe_kotoba(&src, &Policy::deny_all()));
    }
}

#[test]
fn bitwise_ops_on_numbers_compile() {
    // Correct usage — no false positive.
    let wasm = compile_safe_kotoba(
        r#"(defn run [] (bit-or (bit-shift-left 6 2) (bit-and 5 3)))"#,
        &Policy::deny_all(),
    )
    .expect("bitwise ops on numeric literals must compile");
    assert!(wasm.starts_with(b"\0asm"));
}
...
fn byte_at_on_string_literal_with_numeric_index_is_fine() {
    ...
    let wasm = compile_safe_kotoba(r#"(defn run [] (byte-at "hi" 0))"#, &Policy::deny_all())
        .expect("byte-at on a string literal with a numeric index must compile");
    assert!(wasm.starts_with(b"\0asm"));
}

// ── static bounds check on a literal string + literal index (T1) ────────────

#[test]
fn byte_at_out_of_bounds_literal_index_is_rejected() {
    // index >= len, index == len, and an empty string are all OOB reads.
    for src in [
        r#"(defn run [] (byte-at "ab" 5))"#, // past the end
        r#"(defn run [] (byte-at "ab" 2))"#, // == len, still OOB (valid 0..2)
        r#"(defn run [] (byte-at "" 0))"#,   // empty string has no byte 0
    ] {
        denied_type(compile_safe_kotoba(src, &Policy::deny_all()));
    }
}
...
#[test]
fn byte_at_negative_literal_index_is_rejected() {
    denied_type(compile_safe_kotoba(
        r#"(defn run [] (byte-at "ab" -1))"#,
        &Policy::deny_all(),
    ));
}

#[test]
fn byte_at_last_valid_literal_index_compiles() {
    // index == len-1 is the last in-bounds byte.
    let wasm = compile_safe_kotoba(r#"(defn run [] (byte-at "ab" 1))"#, &Policy::deny_all())
        .expect("the last in-bounds index must compile");
    assert!(wasm.starts_with(b"\0asm"));
}

#[test]
fn byte_at_bounds_use_utf8_byte_length() {
    // "é" is two UTF-8 bytes, so index 1 is in bounds and index 2 is not.
    let ok = compile_safe_kotoba(r#"(defn run [] (byte-at "é" 1))"#, &Policy::deny_all())
        .expect("index 1 of a 2-byte string must compile");
    assert!(ok.starts_with(b"\0asm"));
    denied_type(compile_safe_kotoba(
        r#"(defn run [] (byte-at "é" 2))"#,
        &Policy::deny_all(),
    ));
}

#[test]
fn byte_at_non_literal_operands_are_not_bounds_checked() {
    // A variable index or variable string can't be checked statically — these
    // must compile (no false positives; runtime owns the bound there).
    for src in [
        r#"(defn run [i] (byte-at "ab" i))"#, // variable index
        r#"(defn run [s] (byte-at s 9))"#,    // variable string
    ] {
        let wasm = compile_safe_kotoba(src, &Policy::deny_all())
            .expect("non-literal byte-at operands must not be bounds-checked");
        assert!(wasm.starts_with(b"\0asm"));
    }
}
...
fn str_len_on_variable_is_not_flagged() {
    let wasm = compile_safe_kotoba("(defn run [s] (str-len s))", &Policy::deny_all())
...
fn comparison_ops_on_string_literal_rejected() {
    for op in ["<", ">", "<=", ">="] {
        ...
        denied_type(compile_safe_kotoba(&a, &Policy::deny_all()));
        denied_type(compile_safe_kotoba(&b, &Policy::deny_all()));
    }
}
...
fn equality_on_string_literal_is_not_flagged() {
    ...
    let wasm = compile_safe_kotoba(r#"(defn run [n] (= n "x"))"#, &Policy::deny_all())
...
fn numeric_comparison_on_numbers_compiles() {
    let wasm = compile_safe_kotoba("(defn run [n] (if (< n 10) n 0))", &Policy::deny_all())
...
fn division_by_nonzero_literal_compiles() {
    let wasm = compile_safe_kotoba("(defn run [n] (quot n 7))", &Policy::deny_all())
...
fn division_by_variable_is_not_flagged() {
    ...
    let wasm = compile_safe_kotoba("(defn run [n d] (/ n d))", &Policy::deny_all())
...
fn zero_dividend_is_fine() {
    ...
    let wasm = compile_safe_kotoba("(defn run [n] (quot 0 n))", &Policy::deny_all())
...
fn arithmetic_on_numbers_compiles() {
    let wasm = compile_safe_kotoba("(defn run [n] (+ (* n n) (- n 1)))", &Policy::deny_all())
...
fn arithmetic_on_variables_is_not_flagged() {
    ...
    let wasm = compile_safe_kotoba("(defn run [s n] (+ s n))", &Policy::deny_all())
...
fn string_literal_in_string_op_is_fine() {
    ...
    let wasm = compile_safe_kotoba(r#"(defn run [] (str-len "hello"))"#, &Policy::deny_all())
