"""UsDataGovSensor — Wave-1 anchor: ``gov/open-data/usa/data-gov``.

Per ADR-2605263900. data.gov is the US federal government's open-data
catalog (CKAN-based) — ~250K dataset entries across federal agencies
under **public domain** (US federal government works are not
copyrighted per 17 USC 105). Individual datasets MAY carry additional
attribution requirements via their CKAN ``license_id`` field; the
sensor passes that through on the observation.

This sensor reads the per-dataset NDJSON view emitted by the (TODO W1)
``us_data_gov.py`` fetcher. One row per CKAN package → one
``GovOpenDataObservation`` with ``jurisdiction="USA"`` and
``sensor`` field set to this sensor's name.

Sibling of :class:`UkDataGovUkSensor` (UK; W1 deferred) +
:class:`JpDataGoJpSensor` (JP; W1 deferred) — all three would yield
``GovOpenDataObservation`` and disambiguate via the ``jurisdiction``
field. This is the first ``GovOpenDataSensor`` concrete impl
(completes the W1 sensor parity at 10/10).

Hot-sample is deterministic on ``(pin.revision, n)`` per G7.

Per ADR-2605263900 §5 publication-rule honoring: data.gov publisher +
organization fields pass-through (state agencies + public officers are
upstream-named by design; the dataset catalog itself does NOT carry
private PII). GDPR-class right-to-be-forgotten DSARs route through
``chigiri.data_privacy`` to upstream publisher (GSA / individual
federal agency); religious-corp NEVER performs unilateral removal.

Passive-only invariant: NEVER hits catalog.data.gov CKAN live API at
...
@dataclass
class UsDataGovSensor:
    """Sensor over a ``gov/open-data/usa/data-gov/<rev>/`` subdataset.

    Expected NDJSON row shape (minimum):
      - datasetId          (CKAN package name, e.g. "noaa-climate-data";
                            required)
      - title              (human-readable dataset title; required)
      - license            (per-dataset license ID, e.g.
                            "us-pd" / "cc-by" / "odc-pddl"; required)
      - payloadCid         (IPFS CID of normalized full CKAN package
                            metadata; required)
      - descriptionExcerpt (optional short excerpt; full body in payloadCid)
      - publisher          (optional publishing federal agency)
      - publishedAtUtc     (optional ISO-8601 UTC publication date)
    ...
    (datasetId / title / license / payloadCid) are skipped without
    halting the stream.
    """

    name: str = "gov/open-data/usa/data-gov"
    annex_root: Path = Path("90-docs/baien/datasets")
    pin_resolver: StaticPinResolver = field(default_factory=StaticPinResolver)
    license: str = "public-domain"  # Default sensor-level license; per-row override via row.license
    tier: Tier = "A"
    # data.gov publishes weekly bulk; daily refresh oversamples but
    # avoids missing any tighter-cadence subset.
    ...
    # matches the supplied string (e.g., "noaa-gov" for NOAA-only
    # consumers). "" = all organizations pass.
