"""UsCongressGovSensor — Wave-1 anchor: ``gov/parliament/usa/congress-gov``.

Per ADR-2605263900. Congress.gov + govinfo.gov publish bulk data for
federal bills, roll-call votes, member info, Congressional Record,
and treaty documents under **public domain** (US federal government
works are not copyrighted per 17 USC 105).

This sensor reads the per-record NDJSON view emitted by the (TODO W1)
``us_congress_gov.py`` fetcher across the bulk collections (BILLS,
BILLSTATUS, CREC, FR). One row per record → one
``GovParliamentObservation`` with ``legislature="us-congress"``.

Hot-sample is deterministic on ``(pin.revision, n)`` per G7.

Per ADR-2605263900 §5 publication-rule honoring: Congress.gov
pass-through (members of Congress, witnesses, named petitioners are
upstream-named as the public record). GDPR-class right-to-be-forgotten
DSARs route through ``chigiri.data_privacy`` to upstream publisher
(Library of Congress / govinfo.gov); religious-corp NEVER performs
unilateral removal.

Passive-only invariant: NEVER hits api.congress.gov live API at
organism-tick time. Only reads pre-fetched IPFS-pinned subdataset
(govinfo bulkdata sync). Vendor commercial gov-intel terminal imports
(GovWin IQ / Bloomberg Government / Politico Pro / E&E News Pro /
FiscalNote / CQ Roll Call Pro) are CONSTITUTIONALLY PROHIBITED per
Charter Rider §2(e)+§2(c).
"""

from __future__ import annotations

import json
import random
from dataclasses import dataclass, field
from pathlib import Path
from typing import Iterator

from ..base import DatasetPin, PiiFilterPolicy, StaticPinResolver, Tier
from .base import GovParliamentObservation, ParliamentRecordKind


# Congress.gov / govinfo.gov native record-type strings → canonical
# ParliamentRecordKind enum. Sources: govinfo.gov collection
# documentation + Congress.gov bulk-data taxonomy.
_RECORD_KIND_MAP: dict[str, ParliamentRecordKind] = {
    # Bills (BILLS + BILLSTATUS collections)
    "Bill": "bill",
    "Bill Text": "bill",
    "Bill Status": "bill",
    "House Bill": "bill",
    "Senate Bill": "bill",
    "Joint Resolution": "bill",
    "Concurrent Resolution": "bill",
    "Simple Resolution": "bill",
    # Floor debates (CREC collection)
    "House Floor Debate": "debate",
    "Senate Floor Debate": "debate",
    "House Debate": "debate",
    "Senate Debate": "debate",
    # Committees
    "Committee Report": "committee",
    "Committee Hearing": "committee",
    "Committee Meeting": "committee",
    "Committee Print": "committee",
    # Votes
    "Roll Call": "vote",
    "Roll Call Vote": "vote",
    "Vote": "vote",
    "Recorded Vote": "vote",
    # Member statements + extensions
    "Floor Statement": "member-statement",
    "Extension of Remarks": "member-statement",
    "Written Statement": "member-statement",
    # Petitions (rare in US Congress but recorded)
    "Petition": "petition",
    "Memorial": "petition",
    # Questions (rare; reserved for forward-compat)
    "Question": "question",
}

# Canonical US chamber values.
_CHAMBER_VALUES: set[str] = {"House", "Senate"}


def _canonical_record_kind(native: str) -> ParliamentRecordKind | None:
    """Map a Congress.gov native record-type string to canonical enum.

    Returns ``None`` if unknown — sensor skips per G7 schema discipline.
    """
    return _RECORD_KIND_MAP.get(native)


@dataclass
class UsCongressGovSensor:
    """Sensor over a ``gov/parliament/usa/congress-gov/<rev>/`` subdataset.

    Expected NDJSON row shape (minimum):
      - recordId           (per-record canonical ID; e.g. "BILLS-119hr1234ih"; required)
      - sessionDateUtc     (ISO-8601 UTC; required)
      - payloadCid         (IPFS CID of normalized payload; required)
      - chamber            ("House" | "Senate"; required)
      - recordKind OR nativeKind  (canonical kind OR native string for
                                   canonical mapping; required)
      - congress           (optional Congress number, e.g. 119)
      - bodyExcerpt        (optional short excerpt; full body in payloadCid)
      - speakerName        (optional pass-through per §5)
      - speakerRole        (optional; e.g. "Senator", "Representative",
                            "Speaker of the House", "Majority Leader")

    G7 schema discipline: rows missing required fields OR with
    unmappable nativeKind (and no explicit recordKind) are skipped
    without halting the stream.
    """

    name: str = "gov/parliament/usa/congress-gov"
    annex_root: Path = Path("90-docs/baien/datasets")
    pin_resolver: StaticPinResolver = field(default_factory=StaticPinResolver)
    license: str = "public-domain"
    tier: Tier = "A"
    # Congress.gov publishes within ~24h of sessions; daily refresh.
    refresh_cadence_sec: int = 24 * 3600
    pii_filter: PiiFilterPolicy = PiiFilterPolicy.STRICT
    jurisdiction_iso3: str = "USA"
    legislature: str = "us-congress"
    # Optional filter: only yield records from a specific chamber
    # ("House" or "Senate"). "" = both chambers pass.
    chamber_filter: str = ""
    # Optional record-kind filter (canonical enum values).
    record_kind_filter: tuple[ParliamentRecordKind, ...] = ()
    ndjson_suffix: str = ".ndjson"
    _cached_pin: DatasetPin | None = field(default=None, init=False, repr=False)

    def latest_pin(self) -> DatasetPin:
        pin = self.pin_resolver.latest(self.name)
        self._cached_pin = pin
        return pin

    def _resolve_ndjson_paths(self, pin: DatasetPin) -> list[Path]:
        subdataset_dir = self.annex_root / self.name
        if not subdataset_dir.exists():
            raise FileNotFoundError(
                f"subdataset '{self.name}' not present at {subdataset_dir}"
            )
        candidates = sorted(
            (p for p in subdataset_dir.iterdir() if p.is_dir()),
            reverse=True,
        )
        if not candidates:
            raise FileNotFoundError(f"no snapshot directory under {subdataset_dir}")
        snapshot_dir = candidates[0]
        ndjson_files = sorted(snapshot_dir.glob(f"*{self.ndjson_suffix}"))
        if not ndjson_files:
            raise FileNotFoundError(
                f"no '*{self.ndjson_suffix}' under {snapshot_dir}"
            )
        return ndjson_files

    def stream(self, pin: DatasetPin) -> Iterator[GovParliamentObservation]:
        sensor_name = self.name
        chamber_filter = self.chamber_filter
        kind_filter_set = (
            set(self.record_kind_filter) if self.record_kind_filter else None
        )
        for path in self._resolve_ndjson_paths(pin):
            with path.open("r", encoding="utf-8") as f:
                for line in f:
                    s = line.strip()
                    if not s:
                        continue
                    try:
                        row = json.loads(s)
                    except json.JSONDecodeError:
                        continue
                    record_id = str(row.get("recordId", "")).strip()
                    session_date_utc = str(row.get("sessionDateUtc", "")).strip()
                    payload_cid = str(row.get("payloadCid", "")).strip()
                    chamber = str(row.get("chamber", "")).strip()
                    if not (record_id and session_date_utc and payload_cid and chamber):
                        # G7 — required field missing.
                        continue
                    if chamber not in _CHAMBER_VALUES:
                        # G7 — unknown chamber value.
                        continue
                    if chamber_filter and chamber != chamber_filter:
                        continue
                    # Resolve canonical record kind.
                    record_kind_raw = row.get("recordKind")
                    record_kind: ParliamentRecordKind | None
                    if isinstance(record_kind_raw, str) and record_kind_raw:
                        record_kind = record_kind_raw  # type: ignore[assignment]
                    else:
                        native_kind = str(row.get("nativeKind", "")).strip()
                        record_kind = _canonical_record_kind(native_kind)
                    if record_kind is None:
                        # G7 — kind unresolvable; skip.
                        continue
                    if kind_filter_set is not None and record_kind not in kind_filter_set:
                        continue
                    yield GovParliamentObservation(
                        sensor=sensor_name,
                        tier=self.tier,
                        pin_revision=pin.revision,
                        jurisdiction_iso3=self.jurisdiction_iso3,
                        legislature=self.legislature,
                        record_kind=record_kind,
                        record_id=record_id,
                        body_excerpt=str(row.get("bodyExcerpt", ""))[:8192],
                        speaker_name=(
                            str(row["speakerName"])
                            if "speakerName" in row and row["speakerName"]
                            else None
                        ),
                        speaker_role=(
                            str(row["speakerRole"])
                            if "speakerRole" in row and row["speakerRole"]
                            else None
                        ),
                        session_date_utc=session_date_utc,
                        license_tag=self.license,
                        payload_cid=payload_cid,
                        state_aligned_flag=False,  # USA is not §2(g)-flagged
                        internal_only=False,
                        pii_redacted=bool(row.get("piiRedacted", False)),
                    )

    def hot_sample(self, pin: DatasetPin, n: int) -> list[GovParliamentObservation]:
        """Reservoir sample; deterministic on (pin.revision, n) per G7."""
        rng = random.Random(f"{pin.revision}:{n}")
        reservoir: list[GovParliamentObservation] = []
        for i, obs in enumerate(self.stream(pin)):
            if i < n:
                reservoir.append(obs)
            else:
                j = rng.randint(0, i)
                if j < n:
                    reservoir[j] = obs
        return reservoir


__all__ = ["UsCongressGovSensor"]
