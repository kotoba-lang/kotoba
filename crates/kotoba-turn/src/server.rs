...
pub fn authenticate(msg: &[u8], secret: &str, now: u64) -> Result<Scope, AuthReject> {
    ...
    if !attrs.iter().any(|(t, _)| *t == stun::ATTR_MESSAGE_INTEGRITY) {
        return Err(AuthReject::Unauthorized);
    }
    ...
    stun::verify_message_integrity(msg, credential.as_bytes()).map_err(|_| AuthReject::Unauthorized)?;
...
mod tests {
    ...
    fn method_classification() {
        ...
        assert_eq!(method_of(stun::CREATE_PERMISSION_REQUEST), TurnMethod::CreatePermission);
        assert_eq!(method_of(stun::CHANNEL_BIND_REQUEST), TurnMethod::ChannelBind);
    ...
    fn authed_allocate(secret: &str, room: &str, player: u32, expires_at: u64) -> Vec<u8> {
        ...
        let mut msg = Header { typ: stun::ALLOCATE_REQUEST, length: 0, txid: [7; 12] }.encode().to_vec();
    ...
    fn authenticate_rejects_wrong_secret_and_missing_integrity() {
        ...
        assert_eq!(authenticate(&msg, "WRONG", 1_700_000_000), Err(AuthReject::Unauthorized));

        // A request with no attributes at all → no USERNAME → unauthorized.
        let bare = Header { typ: stun::ALLOCATE_REQUEST, length: 0, txid: [0; 12] }.encode().to_vec();
        assert_eq!(authenticate(&bare, "s3cret", 0), Err(AuthReject::Unauthorized));
    }

    #[test]
    fn authenticate_flags_expired_credentials_as_stale() {
        let msg = authed_allocate("s3cret", "r", 1, 1_700_000_060);
        assert_eq!(authenticate(&msg, "s3cret", 1_700_000_061), Err(AuthReject::Stale));
