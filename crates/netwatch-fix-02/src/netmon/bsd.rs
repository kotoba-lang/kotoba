...
fn create_socket() -> Result<tokio::net::UnixStream> {
    ...
    // socket2::Type::RAW requires feature="all" which netwatch doesn't enable.
    // Use From<c_int> instead (always available) to avoid the feature flag.
    ...
    // socket2::Socket has no From impl for std::os::unix::net::UnixStream;
    // use raw fd conversion which works on all Unix targets.
    let fd = socket.into_raw_fd();
    let socket_std: std::os::unix::net::UnixStream =
