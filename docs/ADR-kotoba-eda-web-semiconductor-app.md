...
- `docs/eda/eda_signoff_evidence.edn` と `:eda.signoff/*` は OpenSTA PVT timing、
  OpenROAD route、KLayout DRC、Netgen LVS、ngspice corner、ATE pattern evidence を
  release-ready gate として扱う。stage fallback だけでは manufacturing signoff にならない。
...
- 現在の成熟度は **alpha evidence pipeline**。CLJC native kernel は prototype、
  OSS report normalization と EDN data contract は CI-verifiable、tapeout/manufacturing signoff は
  evidence shape のみであり、foundry PDK/rule deck、実 OSS tool 実行、waiver review、
  corner/PVT matrix、human approval が必要。
...
- Repo: `kotoba-lang/kotoba` に web workbench、`kotoba-lang/eda` に native CLJC kernel / OSS report normalizer。
- License: kotoba-lang 独自 repo は Apache-2.0 既定。upstream 由来 repo は upstream license / NOTICE を維持。
- UI source: Hiccup + shadow-css data を正本にする。静的 HTML は生成物または互換配信物として扱う。
- Verification: CLJC native kernel は prototype。OSS report normalizer は CI-verifiable。
- Release stance: manufacturing/tapeout release-ready は stage fallback では不可。実 tool evidence、corner/PVT、
  DRC/LVS/STA/SPICE/ATE、waiver、人間承認が揃うまで `Not signoff-ready` とする。

## Remaining Open Questions

1. private PDK を vault/policy 前提で扱う時の tenant boundary と audit log retention。
2. 実 OpenROAD/OpenSTA/KLayout/Netgen/ngspice を GitHub-hosted runner に入れるか、self-hosted runner に限定するか。
3. LLM provider は murakumo-hosted model を既定にし、外部 provider は opt-in にするか。
