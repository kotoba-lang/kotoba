...
```bash
kbb -M:dev:test   # wasm_exec_test, real_host_providers_test, actor_host_test, …
```
...
| [`examples/kbb/env_browse_proc.kotoba`](../examples/kbb/env_browse_proc.kotoba) | the three non-fs modules in one script: entry count of one directory (`kbb.browse`), whether `HOME` is set (`kbb.env`), exit status of `/bin/echo kbb-js-proc-ok` (`kbb.proc`, the one granted program). | `entries * 1000 + (HOME set? 100 : 0) + exit`; the test recomputes `entries` from the fixture dir at run time and asserts required capabilities `[20 33 34]` | `kbb_js_test.clj` |
...
| `probe_proc_via_env` | `:proc/exec` (wire 20) with the request `"<program>[ARG_SEP<arg>]..."` from `KBB_PROBE_REQUEST` | exit status, or `:denied` by name: `:spawn/program-not-granted`, `:spawn/path-lookup-denied`, `:spawn/argv-budget`, `:spawn/output-budget`, `:spawn/timeout` | `kbb_js_providers_test.clj` |
...
| [`kbb.proc`](../lib/kbb/proc.kotoba) | `:process/spawn` — `run` / `exec` / `ok?`: ONE granted program, `"<program>[ARG_SEP<arg>]..."` (kotoba-lang `lang/capability-catalog.edn` `:process/spawn`) | 20 | js; the kexe loader and the Linux static ELF answer the same grammar (the shim routes it to js) |
