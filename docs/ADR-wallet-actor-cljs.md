...
R0 landed as pure `.cljc` namespaces under `crates/kotoba-wasm/web/cljs/src/kotoba/wallet/`:
...
- `shadow-cljs.edn` includes ESM exports for the wallet provider/runtime wrappers and `npm run test:wallet:all` verifies Node 22+ runtime alignment, pure tests, ADR document/registry/note status sync, ADR artifact consistency, README gate discoverability, package-lock metadata, CI wallet job shape, shadow-cljs wallet export consistency, browser bundle compile, and generated ESM smoke; the root README and CLJS README link this wallet maturity gate, and CI runs it as an independent wallet CLJS job.
...
- Browser/CLJS ESM export wrappers and export map: **R0.3 landed**.
- Stateful EIP-1193 provider object wrapper: **R0.4 landed**; browser bundle verification **R0.33 landed** with `npx shadow-cljs compile web` completing at 0 warnings; JS-facing wallet ESM smoke **R0.34 landed**; runtime ESM effect/command normalization **R0.35 landed**; ESM sign+submit and quote host smoke **R0.36 landed**; ESM sync materialization smoke **R0.37 landed**; unified wallet maturity gate **R0.38 landed** as `npm run test:wallet:all`; EDN parse gate coverage **R0.39 landed**; root README gate discoverability **R0.40 landed**; CI wallet gate **R0.41 landed**; ADR registry consistency gate **R0.42 landed**; strict signature address params **R0.43 landed**; RPC tx-object policy guard **R0.44 landed**; registered-chain provider guard **R0.45 landed**; registered-account provider guard **R0.46 landed**; ESM provider guard error smoke **R0.47 landed**; ADR status sync gate **R0.48 landed**; Node 22+ runtime guard **R0.49 landed**; CI wallet job shape gate **R0.50 landed**; package-lock metadata gate **R0.51 landed**; root README gate summary check **R0.52 landed**; CLJS README gate summary check **R0.53 landed**; add-chain payload validation **R0.54 landed**; add-chain ESM smoke **R0.55 landed**; host-effect result fallback **R0.56 landed**; provider event ESM smoke **R0.57 landed**; listener exception isolation **R0.58 landed**; idempotent removeListener cleanup **R0.59 landed**; provider effect namespace preservation **R0.60 landed**; host result re-normalization **R0.61 landed**; host-effect error conversion **R0.62 landed**; effect-free host handler skip **R0.63 landed**; watch-asset payload validation **R0.64 landed**; setState replacement events **R0.65 landed**; runtime effect error conversion **R0.66 landed**; command replay error conversion **R0.67 landed**; runtime composite-key state round-trip **R0.68 landed**; provider composite-key state round-trip **R0.69 landed**; direct walletRequest error conversion **R0.70 landed**; provider state commit atomicity **R0.71 landed**; hex chain id JS state normalization **R0.72 landed**; hex policy chain JS state normalization **R0.73 landed**; address policy account JS state normalization **R0.74 landed**; address selected account JS state normalization **R0.75 landed**; method-name cap JS state normalization **R0.76 landed**; setState structured error conversion **R0.77 landed**; selected-chain state error conversion **R0.78 landed**; selected-chain fallback error conversion **R0.79 landed**; selected-account fallback error conversion **R0.80 landed**; kebab chain-id request normalization **R0.81 landed**; runtime chain-id payload normalization **R0.82 landed**; address accountId request normalization **R0.83 landed**; explicit from/address account and chainId intent materialization **R0.84 landed**; nested swap request account/chain materialization **R0.85 landed**; signature address account materialization **R0.86 landed**; async runtime host effects **R0.87 landed**; async runtime host success coverage **R0.88 landed**; async provider host success coverage **R0.89 landed**; async provider result-only success coverage **R0.90 landed**; async malformed host state non-commit **R0.91 landed**; rejecting host-effect non-commit **R0.92 landed**; throwing host-effect non-commit **R0.93 landed**; host-effect tx error provenance **R0.94 landed**; host-effect tx JS error message **R0.95 landed**; host-effect tx structured code **R0.96 landed**; provider host command replay **R0.97 landed**; malformed provider host command replay non-commit **R0.98 landed**; provider host command replay events **R0.99 landed**; provider host command replay listener isolation **R0.100 landed**; provider host command replay event dedupe **R0.101 landed**; pure provider event derivation coverage **R0.102 landed**; pure setState event derivation coverage **R0.103 landed**; pure malformed event selected-chain coverage **R0.104 landed**; pure command replay partial non-commit coverage **R0.105 landed**; runtime sync explicit command ordering coverage **R0.106 landed**; runtime quote mismatch provenance coverage **R0.107 landed**; fatal quote mismatch state evidence coverage **R0.108 landed**; fatal expired quote state evidence coverage **R0.109 landed**; signature payload privacy coverage **R0.110 landed**; intent hash payload digest binding coverage **R0.111 landed**; signature observation payload-hash replay coverage **R0.112 landed**; tx-signed signed-raw replay coverage **R0.113 landed**; tx-confirmed block-number replay coverage **R0.114 landed**; tx-confirmed block-number validation coverage **R0.115 landed**; tx-confirmed gas-used validation coverage **R0.116 landed**; tx-submitted submitted-at validation coverage **R0.117 landed**; tx hex-prefix replay coverage **R0.118 landed**; signature hex-prefix replay coverage **R0.119 landed**; runtime host hex-prefix validation coverage **R0.120 landed**; runtime host timestamp validation coverage **R0.121 landed**; runtime quote timestamp validation coverage **R0.122 landed**; balance/allowance observation validation coverage **R0.123 landed**; tx-confirmed confirmed-at validation coverage **R0.124 landed**; actor network/asset command validation coverage **R0.125 landed**; actor connect command validation coverage **R0.126 landed**; actor intent preparation command validation coverage **R0.127 landed**; actor signature preparation command validation coverage **R0.128 landed**; actor state transition command validation coverage **R0.129 landed**; actor sync command validation coverage **R0.130 landed**; JS command tuple validation coverage **R0.131 landed**; JS command batch validation coverage **R0.132 landed**; provider host-error-data coverage **R0.133 landed**; provider null host result guard coverage **R0.134 landed**; JS state object guard coverage **R0.135 landed**; runtime JS state object guard coverage **R0.136 landed**; runtime JS effect object guard coverage **R0.137 landed**; runtime JS env object guard coverage **R0.138 landed**; runtime capability function guard coverage **R0.139 landed**; provider constructor env guard coverage **R0.140 landed**; provider handleEffects function guard coverage **R0.141 landed**; provider origin string guard coverage **R0.142 landed**; provider listener function guard coverage **R0.143 landed**; provider event string guard coverage **R0.144 landed**; provider removeListener event guard coverage **R0.145 landed**; provider request object guard coverage **R0.146 landed**; provider request method guard coverage **R0.147 landed**; provider request params guard coverage **R0.148 landed**; direct walletRequest shape guard coverage **R0.149 landed**; provider missing origin guard coverage **R0.150 landed**; provider blank origin guard coverage **R0.151 landed**; direct walletRequest origin guard coverage **R0.152 landed**; provider blank event guard coverage **R0.153 landed**; provider removeListener listener guard coverage **R0.154 landed**; provider setState method boundary coverage **R0.155 landed**; JS policy origin guard coverage **R0.156 landed**; JS accounts object guard coverage **R0.157 landed**; JS networks object guard coverage **R0.158 landed**; JS policies object guard coverage **R0.159 landed**; JS record map object guard coverage **R0.160 landed**; JS composite map object guard coverage **R0.161 landed**; runtime JS accounts object guard coverage **R0.162 landed**; runtime JS networks object guard coverage **R0.163 landed**; runtime JS policies object guard coverage **R0.164 landed**; runtime JS record map object guard coverage **R0.165 landed**; runtime JS composite map object guard coverage **R0.166 landed**; runtime host result object guard coverage **R0.167 landed**; runtime optional capability function guard coverage **R0.168 landed**.
