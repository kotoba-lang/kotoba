...
16. **M16 deploy auto-publish** ◐: `kotoba-lattice::deploy::deploy_messages(app, resolved)` — manifest を lattice 制御メッセージ(`PutTriggers` datom-Δ + `PutRoutes` KSE/cron/HTTP, CMD topic)に変換(純粋, 4 tests)。XRPC `mesh.deploy`(p2p gate)が manifest EDN を受け取り deploy_messages を構築 → `gossip_tx` で lattice へ publish + ローカル `mesh_routes` を即時更新。CLI `kotoba app deploy --publish` が manifest を POST(従来の dry-run に control message plan の表示も追加)。これで「書く→deploy→全 trigger 自動実行」が閉じた。**残り(R0)**: publish は `:cid` 明示 manifest 向け(`:src` はサーバ側 compile 不可のため事前 `kotoba component build` が必要)。複数 trigger 同居の実行時 dispatch 最適化は将来。
