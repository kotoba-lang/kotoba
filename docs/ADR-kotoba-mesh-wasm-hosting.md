...
3. **M3 server 参加 + CLI + ビルド** ✅: (1) `kotoba-server::net_actor` の swarm event loop に lattice 参加（`subscribe_lattice` + 定期 Heartbeat publish + auction 自動入札）。(2) `kotoba component build`（`.kotoba`→ Kotoba 既定で wasm component → 正準 CID、`.clj`/`.cljc`/`.cljs` は互換入力）。(3) `kotoba app deploy`（EDN manifest を各 component コンパイルして content-addressed desired state に解決）/ `kotoba lattice ps`。**残り**: `StartComponent → WasmExecutor` の実行配備 + desired-state を control graph から各ノードへ。
...
| component logic | **Kotoba**（`.kotoba` canonical, `.clj`/`.cljc`/`.cljs` compat）→ wasm component（`kotoba-node` world） | `kotoba component build` (`kotoba-clj::component::compile_kais_component_str`) |
...
*.cljs   → kotoba component build / compile_kais_component_str(src, runtime_wit_dir) ← compat
