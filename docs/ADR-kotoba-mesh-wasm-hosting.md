...
10. **M10 combined kotoba-mesh world** ◐: 1 component で複数 trigger（run + on-http + on-tick + on-kse）を同居。新 WIT world `kotoba-mesh`（4 export 全部）。guest が定義した handler はそのまま、未定義のものは **AST に空 `(defn …)` stub を合成**して常に world を満たす（`compile_kais_mesh` が trigger 2 個以上で kotoba-mesh を選択、1 個なら従来の専用 world、0 なら kotoba-node）。run+on-http+on-tick+on-kse 全部入り / 2 個+stub の component が wasmtime ロード可能を実証。**残り**: host 側の実行時 dispatch（net_actor/server が trigger 種別で `on-http`/`on-tick`/`on-kse` export を呼び分け、WasmExecutor は現状 `run` のみ）は M11。
