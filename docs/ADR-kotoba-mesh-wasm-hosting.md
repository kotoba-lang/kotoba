...
3. **M3 server 参加 + CLI + ビルド** ✅: (1) `kotoba-server::net_actor` の swarm event loop に lattice 参加（`subscribe_lattice` + 定期 Heartbeat publish + auction 自動入札）。(2) `kotoba component build`（`.kotoba`→ Kotoba 既定で wasm component → 正準 CID、`.clj`/`.cljc` は互換入力、ClojureScript-targeted behavior は `.cljc` reader conditional）。(3) `kotoba app deploy`（EDN manifest を各 component コンパイルして content-addressed desired state に解決）/ `kotoba lattice ps`。**残り**: `StartComponent → WasmExecutor` の実行配備 + desired-state を control graph から各ノードへ。
...
7. **M7 multi-export codegen** ◐: `kotoba component` / Kotoba compiler の `compile_core` を複数 entry 対応に一般化（`Entry.export_name`）+ `compile_kais_mesh_component_str` が `run` に加え `(defn on-http …)` 定義時に `on-http` も export（world を `kotoba-component`(run+on-http) / `kotoba-node`(run のみ)に自動選択）。新 WIT world `kotoba-component` 追加。run+on-http の component が wasmtime でロード可能を実証(tests/mesh_component)。
8. **M8 on-tick (cron) trigger** ◐: 新 `EntryAbi::I64ToResultBytes` + `tick_wrapper_fn`（`(epoch:i64)->result<list<u8>,string>` の canonical-ABI wrapper、epoch を guest fn に値渡し）。`compile_core` の wrapper type を entry 毎に ABI 別シグネチャで生成。新 WIT world `kotoba-cron`(run+on-tick)。`(defn on-tick [epoch] …)` 定義で `kotoba-cron` を自動選択。run+on-tick の component が wasmtime ロード可能を実証。
9. **M9 on-kse (KSE topic) trigger** ◐: 新 `EntryAbi::StringBytesToResultBytes` + `kse_wrapper_fn`（`(topic_ptr,topic_len,payload_ptr,payload_len)->result` の 2 引数 wrapper、2 つの handle を構築して arity-2 guest fn に渡す）。`EntryAbi::guest_arity()` を追加し `compile_core` が entry を ABI 由来の arity で解決（on-kse は arity-2 lookup）。新 WIT world `kotoba-kse`(run+on-kse)。`(defn on-kse [topic payload] …)` 定義で `kotoba-kse` を自動選択。run+on-kse の component が wasmtime ロード可能を実証。
10. **M10 combined kotoba-mesh world** ◐: 1 component で複数 trigger（run + on-http + on-tick + on-kse）を同居。新 WIT world `kotoba-mesh`（4 export 全部）。guest が定義した handler はそのまま、未定義のものは **AST に空 `(defn …)` stub を合成**して常に world を満たす（`compile_kais_mesh` が trigger 2 個以上で kotoba-mesh を選択、1 個なら従来の専用 world、0 なら kotoba-node）。run+on-http+on-tick+on-kse 全部入り / 2 個+stub の component が wasmtime ロード可能を実証。
11. **M11 host trigger dispatch** ◐: `WasmExecutor` の `run` 呼び出しを汎用 `invoke(export_name, args: Vec<Val>)` に抽出し、trigger 別メソッド `execute_on_http(req)` / `execute_on_tick(epoch:u64)` / `execute_on_kse(topic, payload)` を追加（dynamic Val dispatch、host setup・gas・snapshot・result 解析を共有）。**end-to-end 実証**: 1 つの kotoba-mesh component に run/on-http/on-tick/on-kse を定義し、各メソッドが対応 export を実行して別々の戻り値(RUN/HTTP/TICK/KSE)を返すことをテスト。未定義 export 呼び出しは clean に Err。これで「component を書く→ trigger 種別で実行」が runtime レベルで閉じた。**残り**: net_actor/server が実イベント（KSE gossip→on-kse、cron→on-tick、HTTP route→on-http）でこれらを駆動する配線は M12。
12. **M12 サーバ側 trigger ディスパッチャ** ◐: `net_actor` に `ComponentTrigger`(Run/Http/Tick/Kse) + 統一 `invoke_trigger(cid, trigger)` を追加 — artifact を CID で取得し M11 の `execute*` メソッドへ振り分ける**単一の実行経路**。`start_component`(M4 配備) を `invoke_trigger(Run)` で再構築。全 trigger variant の防御パス（artifact 不在/不正 CID/非 wasm）を net_actor テストで被覆。**残り**: 実イベント源の配線（HTTP route→Http / cron timer→Tick / KSE gossip→Kse）と per-component trigger レジストリ伝播は M13。
13. **M13 イベント源配線** ◐: `kotoba-lattice::routes::TriggerRoutes` — manifest の TriggerSpec を **KSE topic→component / cron→component / HTTP route→component** の routing index に変換（純粋・テスト済）。`LatticeMessage::PutRoutes` で伝播。`net_actor` 配線: PutRoutes で routes 登録 + KSE topic を gossipsub subscribe、**KSE gossip→`invoke_trigger(Kse)`**（hosted な routed component のみ）、**cron tick→`invoke_trigger(Tick)`**（hosted な cron component を heartbeat interval で発火）。これで実ネットワークの KSE/cron イベントが実コンポーネントを駆動。**残り**: HTTP route→`invoke_trigger(Http)` は XRPC server 層（axum）での配線、cron の細粒度スケジュール解析、`kotoba app deploy` からの PutRoutes 自動 publish。
14. **M14 cron 細粒度スケジュール** ◐: `routes::parse_schedule_ms` — `"every 5m"`/`"30s"`/`"2d"` 等を発火間隔(ms)に解析(純粋, 2 tests)。`net_actor` の cron 発火を schedule-aware 化(component 毎に last-fire を追跡し、間隔経過時のみ on-tick 発火、未解析は 60s 既定)。これまで 5s 一律だった cron が schedule 通りに発火。**残り**: HTTP route→`invoke_trigger(Http)` の XRPC/axum 配線(KotobaState 共有 routes + p2p gate)と `kotoba app deploy`→`PutRoutes` 自動 publish は M15。
15. **M15 HTTP route trigger** ◐: 最後の trigger 源を配線。`KotobaState.mesh_routes`(共有 `Arc<RwLock<TriggerRoutes>>`, p2p gate)を net_actor が `PutRoutes` で更新、HTTP layer が読む。`invoke_trigger` を `Option<Vec<u8>>`(成功時 output_cbor)に一般化。axum handler `mesh_http`(`POST /mesh/http/<route>`)が route→component を引き、`invoke_trigger(Http(body))` で `on-http` を実行し**出力を HTTP response として返す**。default(非 p2p)build はクリーンに cfg-out。**残り**: `kotoba app deploy`→`PutRoutes`/`PutTriggers` 自動 publish(deploy→ルート伝播の自動化)は M16。
16. **M16 deploy auto-publish** ◐: `kotoba-lattice::deploy::deploy_messages(app, resolved)` — manifest を lattice 制御メッセージ(`PutTriggers` datom-Δ + `PutRoutes` KSE/cron/HTTP, CMD topic)に変換(純粋, 4 tests)。XRPC `mesh.deploy`(p2p gate)が manifest EDN を受け取り deploy_messages を構築 → `gossip_tx` で lattice へ publish + ローカル `mesh_routes` を即時更新。CLI `kotoba app deploy --publish` が manifest を POST(従来の dry-run に control message plan の表示も追加)。これで「書く→deploy→全 trigger 自動実行」が閉じた。**R0 hardening 済**: (1) publish は CLI が **resolved messages**(compiled CID 込みの `Vec<(topic, LatticeMessage_cbor)>` を CBOR で POST)を送るようになり `:src` component も正しい artifact CID で deploy 可能。(2) **lattice inject channel** 追加 — deploy 元ノードが自分の gossip を受け取れない(gossipsub no-loopback)問題を解消: `mesh.deploy` が各メッセージを gossip(peers)+ inject(self)へ fan-out し、net_actor の `apply_deploy_msg`(gossip/inject 共用)が triggers/routes install + KSE topic subscribe を行う。これで単一ノードでも KSE/cron/HTTP が自己 deploy で発火。複数 trigger 同居の実行時 dispatch 最適化は将来。
...
## 14. デフォルト言語 = Kotoba / EDN / Datomic（`kotoba component build`）

KOTOBA Mesh の**第一級（default）コンポーネント言語は Kotoba**（`kotoba component build`
で WASM Component にコンパイル）とする。理由は「一言語族で manifest・logic・data が揃う」こと:

| 層 | 形式 | 既存実装 |
|---|---|---|
| アプリ manifest | **EDN**（`kotoba.app.edn`） | `kotoba-edn` reader |
| component logic | **Kotoba**（`.kotoba` canonical, `.clj`/`.cljc` compat; `.cljs` source files retired）→ wasm component（`kotoba-node` world） | `kotoba component build` (`kotoba-clj::component::compile_kais_component_str`) |
| データモデル | **Datomic**（datom / Datalog） | `kotoba-datomic` / `kotoba-query` |
| host-import | `kqe`/`kse`/`auth`/`llm`/`chain`（Kotoba builtins） | Kotoba compiler path C-5 `kqe-assert!`/`kqe-query`/`llm-infer` 済み |

つまり **manifest を書く言語＝component を書く言語＝クエリ言語が Kotoba/EDN/Datalog で一つの言語ファミリ**。
...
end-to-end で成立している（Kotoba/WASM component 経路の実績）。

### 14.1 既定ビルド経路

`kotoba component build` は **拡張子で言語を判定し、`.kotoba` を既定**とする
（`.clj` / `.cljc` は互換入力。ClojureScript-targeted behavior は `.cljc` reader conditional）:

```
*.kotoba → kotoba component build / compile_kais_component_str(src, runtime_wit_dir) ← default
*.clj    → kotoba component build / compile_kais_component_str(src, runtime_wit_dir) ← compat
*.cljc   → kotoba component build / compile_kais_component_str(src, runtime_wit_dir) ← compat
*.rs     → cargo component build (wasm32-wasip2)                        ← opt-in
*.py     → componentize-py                                              ← opt-in
*.js     → jco componentize                                             ← opt-in
```

manifest の component 宣言は `:lang` 省略時 `:kotoba`、`:src` を `.kotoba` ファイルにできる:

```clojure
{:name "reply" :src "reply.kotoba"       ; :lang 省略 = :kotoba（default）
 :scale 1 :triggers [{:type :http :route "/reply"}]
 :requires [:cap/kqe :cap/llm]}
```

deploy 時に host 側 / CLI 側で `.kotoba` をコンパイル → 得た wasm の CID を
`:kotoba.component/cid` に確定（content-addressed なので同一 `.kotoba` → 同一 CID、再現可能）。

### 14.2 既定 component の最小形（Kotoba）

```clojure
;; reply.kotoba — KOTOBA Mesh default-language component
(ns reply)
...
component は §4.1 の `on-http`/`on-kse`/`on-tick` を追加 export する（Kotoba compiler の
component export 機構を `run` 同様に拡張＝M1 のスコープ）。

### 14.3 制約（honest R0）

- Kotoba/WASM 言語サブセット（loops/bytes/CBOR は実装済み、C-5 で kqe builtins 済み）で
  書ける範囲が default の到達範囲。複雑な component は Rust/Py に opt-out する逃げ道を残す。
- `on-http` などの追加 export は Kotoba compiler 側の component-export 拡張が前提（M1 タスク）。
